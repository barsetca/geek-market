...
public class OrderController {
    ...
    public String save(Principal principal, @RequestParam Map<String,String> params) {
        String username = principal.getName();
        User user = userService.getByUsername(username).orElseThrow(() -> new UsernameNotFoundException(String.format("User by username %s not exists", username)));
        Order order = new Order(user, cart, params.get("receiver"), params.get("phone"), params.get("address"));
